...
from app.db.session_simple import get_db
...
from app.crud.crud_memory_simple import get_all_memories, clear_character_memories
from app.core.character_generator_simple import SimpleCharacterGenerator
from app.core.memory_generator_simple import SimpleMemoryGenerator
from app.core.response_flow_simple import SimpleResponseFlow

router = APIRouter()

# 初始化核心组件
character_generator = SimpleCharacterGenerator()
memory_generator = SimpleMemoryGenerator()
response_flow = SimpleResponseFlow()
...
async def generate_character(
    ...
    try:
        ...
        # 生成记忆
        memory_generator.generate_memories(db_character, num_memories=6)
...
def get_system_status():
    ...
    return {
        "status": "running",
        "mode": "demo",
        "description": "简化版演示系统，使用模拟数据和模板响应",
        "features": [
            "角色生成（基于模板）",
            "记忆管理（内存存储）",
            "三阶段对话流程（模拟响应）",
