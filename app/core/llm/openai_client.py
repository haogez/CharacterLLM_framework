...
class CharacterLLM:
    ...
    async def generate_character(
        ...
        description: str,
        *,
        enforce_protagonist_relationship: bool = True,
        relationship_override: Optional[str] = None,
        timeline_mode: str = "strict",
    ) -> Dict[str, Any]:
        strict_timeline_rules = """
          \"past_experience\": 角色过往的关键经历，需能解释用户核心设定的成因，经历需具体且与设定有直接因果关系；经历需支撑核心设定（如核心设定 “多疑”，经历为 “曾被亲密朋友背叛、工作中被同事抢功、逐渐不再信任他人”；核心设定 “勇敢”，经历为 “上学时救过落水同学、工作中主动承担高难度任务、面对困难不退缩”）。**重要修改**：必须严格按照格式“0岁时：[具体事件或描述]。1岁时：[具体事件或描述]。2岁时：[具体事件或描述]。3岁时：[具体事件或描述]...直至当前角色年龄”生成，每个年龄的事件具体、详细，并说明对角色的影响。注意：无需每一年都写事件，非关键年份可简略描述（如“4岁时：生活平稳，未发生重大事件”），但必须覆盖从出生到当前年龄的所有年份，且内容不得与“background”字段重复。",
          \"background\": \"角色的背景故事，字数需≥200 字，**重要修改**：必须严格按照 '0岁时：[具体事件+对主角的影响],1岁时：[具体事件+对主角的影响] 2岁时：[具体事件+对主角的影响], 直至当前角色年龄。' 的格式，将 'past_experience' 中的逐年事件进行更详细的展开和阐述。每个年龄的描述必须包含具体的事件、场景、人物互动（如果涉及）、角色的内心感受以及该事件对角色性格、价值观、习惯、行为模式等产生的具体影响。**不允许出现与 'past_experience' 矛盾的内容**，也不允许出现 'past_experience' 中未提及的空泛、概括性描述。例如，如果 'past_experience' 写 '5岁时：父母争吵频繁，开始学会隐藏真实感受'，则 'background' 的 '5岁时：' 部分必须详细描述父母争吵的场景（时间、地点、原因、父母言行）、主角当时的所见所闻所感、她/他如何具体地'学会隐藏'（具体行为、心理活动）、这种经历如何影响了她/他的后续性格或行为（例如，变得警觉、沉默、讨好等）。\",
        """

        relaxed_timeline_rules = """
          \"past_experience\": 角色过往的关键经历，保持与主角设定不矛盾即可，围绕与主角相关的互动或共同经历给出 3-6 条关键节点，允许按阶段概述，不需要逐年罗列；
          \"background\": \"角色背景故事的简要脉络，突出与主角交集或对主角产生影响的情节即可，可用分段描述重要阶段（童年/求学/工作等），无需严格的“X岁时”逐年展开，确保与主角的时间线不冲突且自洽。\",
        """

        timeline_rules = strict_timeline_rules if timeline_mode == "strict" else relaxed_timeline_rules
        ...
        输出格式：严格按JSON格式，21个维度，所有内容用中文，不增减字段、不嵌套（personality除外）。
        ...
        JSON字段及生成规则：
        {
          ...
          "gender": "男/女/其他",  // 取值仅允许 男/女/其他 三类；优先使用用户对角色性的明确设定；用户未设定性别时，可根据角色名称的性别倾向（如 “张伟” 倾向男、“李娜” 倾向女）或核心设定（如 “宝妈” 倾向女）推导
          ...
          "relationship_to_protagonist": "角色与主角的关系，如果是主角本人，则为 '本人'；如果是关联角色，则为 '父亲'、'母亲'、'老师'、'同学'、'朋友' 等。**重要修改**：对于主角本人，此字段必须为 '本人'。"
        }
        ...
        7. 如果角色是主角本人，'relationship_to_protagonist' 字段必须为 '本人'。
        """

        system_prompt = base_prompt.replace("{TIMELINE_RULES}", timeline_rules)
        
        user_prompt = f"基于这句话生成全面角色（用户设定优先）：{description}"

        result = await self.client.generate_structured_response(system_prompt, user_prompt)
        if relationship_override:
            result["relationship_to_protagonist"] = relationship_override
        elif enforce_protagonist_relationship:
            if result.get("relationship_to_protagonist") != "本人":
                print(f"  - 为主角 {result.get('name')} 设置 'relationship_to_protagonist' 为 '本人'")
                result["relationship_to_protagonist"] = "本人"

        # **重要：调整日志输出长度**
        print(f"=== LLM 原始响应 (完整) ===")
        print(result) # 直接打印整个 result 字典或字符串
        print(f"=== 响应类型: {type(result)} ===")
        # ...

        return result # 或处理后的 result
