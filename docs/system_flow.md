...
- 初始化 Neo4j 驱动及 `Neo4jVector`，用于印象节点的向量检索；初始化失败时会降级继续启动。
- 提供 CSV 导入/导出能力：节点、实体-事件关系、时间链、事件细节、角色间关系等均可生成 CSV 并批量导入 Neo4j。
- 支持按角色查询记忆、关系，以及关闭连接的生命周期管理。
...
1) 为主角孵化故事灵感并完善角色背景。
2) 生成分章节故事并作为记忆片段使用。
3) 从故事中抽取实体、关系与时间链；生成 CSV。
4) 将节点、实体-事件关系、时间链、事件细节、角色关系等 CSV 导入 Neo4j。
5) 更新角色的生成状态与耗时统计。
